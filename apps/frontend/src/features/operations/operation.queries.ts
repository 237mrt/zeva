...
export function useCreatePackages() { const client = useQueryClient(); return useMutation({ mutationFn: ({ workOrderId, packages }: { workOrderId: string; packages: PackageInput[] }) => operationApi.createPackages(workOrderId, packages), onSuccess: async (data) => { client.setQueryData(operationKeys.packages(data.workOrder.id), data); await client.invalidateQueries({ queryKey: reportingKeys.all }); } }); }
export function useUpdatePackage() { const client = useQueryClient(); return useMutation({ mutationFn: ({ id, input }: { id: string; input: Partial<PackageInput>; workOrderId: string }) => operationApi.updatePackage(id, input), onSuccess: async (_data, variables) => Promise.all([client.invalidateQueries({ queryKey: operationKeys.packages(variables.workOrderId) }), client.invalidateQueries({ queryKey: reportingKeys.all })]) }); }
export function useDeletePackage() { const client = useQueryClient(); return useMutation({ mutationFn: ({ id }: { id: string; workOrderId: string }) => operationApi.deletePackage(id), onSuccess: async (_data, variables) => Promise.all([client.invalidateQueries({ queryKey: operationKeys.packages(variables.workOrderId) }), client.invalidateQueries({ queryKey: reportingKeys.all })]) }); }
...
export function useCreateDelivery() { const client = useQueryClient(); return useMutation({ mutationFn: (input: CreateDeliveryInput) => operationApi.createDelivery(input), onSuccess: async (delivery) => { client.setQueryData(operationKeys.delivery(delivery.id), delivery); await Promise.all([client.invalidateQueries({ queryKey: operationKeys.all }), client.invalidateQueries({ queryKey: workOrderKeys.all }), client.invalidateQueries({ queryKey: reportingKeys.all })]); } }); }
export function useCancelDelivery() { const client = useQueryClient(); return useMutation({ mutationFn: operationApi.cancelDelivery, onSuccess: async (delivery) => { client.setQueryData(operationKeys.delivery(delivery.id), delivery); await Promise.all([client.invalidateQueries({ queryKey: operationKeys.all }), client.invalidateQueries({ queryKey: workOrderKeys.all }), client.invalidateQueries({ queryKey: reportingKeys.all })]); } }); }

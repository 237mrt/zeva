...
export function useCreateDelivery() { const client = useQueryClient(); return useMutation({ mutationFn: (input: CreateDeliveryInput) => operationApi.createDelivery(input), onSuccess: async (delivery) => { client.setQueryData(operationKeys.delivery(delivery.id), delivery); await Promise.all([client.invalidateQueries({ queryKey: operationKeys.packages(delivery.workOrderId) }), client.invalidateQueries({ queryKey: operationKeys.deliveryLists() }), client.invalidateQueries({ queryKey: workOrderKeys.all })]); } }); }
export function useCancelDelivery() { const client = useQueryClient(); return useMutation({ mutationFn: operationApi.cancelDelivery, onSuccess: async (delivery) => { client.setQueryData(operationKeys.delivery(delivery.id), delivery); await Promise.all([client.invalidateQueries({ queryKey: operationKeys.packages(delivery.workOrderId) }), client.invalidateQueries({ queryKey: operationKeys.deliveryLists() }), client.invalidateQueries({ queryKey: workOrderKeys.all })]); } }); }
